...
std::shared_ptr<SystemUser> StaffRepository::mapRowToUser(const QSqlQuery& query) const {
    ...
    switch (role) {
        case UserRole::Doctor: {
            ...
            return std::make_shared<Doctor>(staffId, staffCode, passwordHash, fullName, role,
                    isActive, specialty, licenseNumber, experienceYears, consultationFee);
        }
            // case UserRole::Nurse: {
...
std::optional<std::shared_ptr<SystemUser>> StaffRepository::findByStaffCode(
        const QString& staffCode) const {
    QString sql = R"(
            SELECT
            s.staff_id,
            s.staff_code,
