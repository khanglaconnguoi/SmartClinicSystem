...
PrescriptionResultDTO PrescriptionRepository::mapRowToPrescriptionHeader(
  ...
  dto.patientAge = dob.isValid() ? (dob.daysTo(QDate::currentDate()) / 365) : 0;
  dto.patientGender = query.value("patient_gender").toString();
...
static const QString SELECT_PRESCRIPTION_SQL = R"(
    SELECT 
        ...
        mr.chief_complaint AS diagnosis,
        p.status, 
        p.notes, 
        p.prescribed_at,
        p.dispensed_by,
...
PrescriptionRepository::findByPatientId(int patientId) const {
  QString sql = R"(
        SELECT 
            ...
            pt.full_name AS patient_name, pt.date_of_birth AS patient_dob, pt.gender AS patient_gender, mr.chief_complaint AS diagnosis,
