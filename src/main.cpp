...
static void seedDatabase() {
  ...
  db.executeQuery(
      "INSERT OR IGNORE INTO departments (department_id, "
...
static void runAllergyInsuranceTests() {
  ...
  bool t1ok = service.AddOutPatient(
      ...
      "Ho Thi Mai", QDate(1992, 7, 20), "Female",
      "079092012345", // CCCD hợp lệ
      "0912000111", "hothi.mai@test.com", "12 Pham Ngu Lao, HCM", "AB+",
      "Penicillin, Aspirin, Pollen", // 3 dị ứng cách nhau bởi dấu phẩy
      "BHYT-123456789",              // số thẻ bảo hiểm
      PatientType::OUTPATIENT, "Nguyen Van Hai", "0900111222");
  ...
  bool t5ok = service.AddOutPatient(
      0, 0, "Phan Van Binh", QDate(1975, 3, 10), "Male", "001075056789",
      "0908777888", "phanvanbinh@test.com", "45 Hang Bai, HN", "O-",
      "", // không có dị ứng
      "", // không có bảo hiểm
      PatientType::OUTPATIENT, "Phan Thi Lan", "0901234000");
  int pid5 = -1;
  if (t5ok) {
  ...
  bool t6ok = service.AddInPatient(
      0, "Le Thi Cam", QDate(1988, 9, 5), "Female", "048088099001",
      "0977555666", "lethicam@test.com", "22 Tran Hung Dao, Da Nang", "B-",
      "Shellfish, Latex", // 2 dị ứng
      "PRIVATE-987654",   // bảo hiểm tư nhân
      PatientType::INPATIENT, "Le Van Dung", "0901222333",
      "1", // roomId
      "1", // doctorId
      QDate::currentDate(), QDate::currentDate().addDays(7), "Appendicitis");
  int pid6 = -1;
  if (t6ok) {
    ...
    if (allergies6.size() == 2) {
      ...
      qDebug() << "[FAIL] Allergies InPatient – expected 2, got:"
               << allergies6.size();
    }
  } else {
  ...
  if (pid1 > 0) {
    ...
    dup.patientId = pid1;
    dup.allergenName =
        "PENICILLIN"; // cùng tên nhưng chữ hoa → COLLATE NOCASE → bị IGNORE
    dup.severity = "SEVERE";
    dupes.append(dup);
    ...
    if (afterDupe.size() == 3) {
      qDebug() << "[PASS] UNIQUE COLLATE NOCASE hoạt động – vẫn còn 3 mục "
                  "(không bị duplicate)";
    } else {
      qDebug() << "[FAIL] Duplicate allergen – expected 3, got:"
               << afterDupe.size();
    }
    Q_UNUSED(insertDupe);
  } else {
