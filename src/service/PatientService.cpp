...
static void populateBaseInsertDTO(PatientInsertDTO &insertDto, const PatientInputDTO &dto, const QString &patientCode) {
...
static void populateBaseUpdateDTO(PatientUpdateDTO &updateDto, const PatientInputDTO &dto, int patientId) {
...
QString PatientService::validateBloodType(const QString &bloodType) {
  ...
  if (!valid.contains(bloodType))
    return "Nhóm máu không hợp lệ. Vui lòng nhập A+, A-, B+, B-, AB+, AB-, O+, O- hoặc UNKNOWN.";
  return "";
}
...
bool PatientService::addInPatient(InPatientInputDTO &dto) {
  ...
  insertDto.dischargeDate = dto.dischargeDate.has_value() ? dto.dischargeDate->toString("yyyy-MM-dd") : "";
...
bool PatientService::updateInPatient(int patientId, InPatientInputDTO &dto,
  ...
  updateDto.dischargeDate = dto.dischargeDate.has_value() ? dto.dischargeDate->toString("yyyy-MM-dd") : "";
