...
parseInsuranceString(int patientId, const QString &raw) {
  ...
  dto.providerName = "BHYT";
  dto.insuranceType = "BHYT";
  dto.coveragePercent = 80.0;
  dto.validFrom = QDate::currentDate().toString("yyyy-MM-dd");
  dto.validTo = QDate::currentDate().addYears(1).toString("yyyy-MM-dd");
  dto.notes = "";
  return dto;
}

static void populateBaseInsertDTO(PatientInsertDTO &insertDto,
                                  const PatientInputDTO &dto,
                                  const QString &patientCode) {
  insertDto.patientCode = patientCode.trimmed();
  insertDto.fullName = dto.fullName.simplified();
  insertDto.dateOfBirth = dto.dateOfBirth.toString("yyyy-MM-dd");
  insertDto.gender = GenderText::toEn(dto.gender);
  insertDto.citizenId = dto.citizenId.trimmed();
  insertDto.phone = dto.phone.trimmed();
  insertDto.email = dto.email.trimmed().toLower();
  insertDto.address = dto.address.simplified();
  insertDto.bloodType = dto.bloodType.trimmed().toUpper();
  insertDto.type = patientTypeToEn(dto.type);
  insertDto.emergencyContactName = dto.emergencyContactName.simplified();
  insertDto.emergencyContactPhone = dto.emergencyContactPhone.trimmed();
}

static void populateBaseUpdateDTO(PatientUpdateDTO &updateDto,
                                  const PatientInputDTO &dto, int patientId) {
  updateDto.patientId = patientId;
  updateDto.fullName = dto.fullName.simplified();
  updateDto.dateOfBirth = dto.dateOfBirth.toString("yyyy-MM-dd");
  updateDto.gender = GenderText::toEn(dto.gender);
  updateDto.citizenId = dto.citizenId.trimmed();
  updateDto.phone = dto.phone.trimmed();
  updateDto.email = dto.email.trimmed().toLower();
  updateDto.address = dto.address.simplified();
  updateDto.bloodType = dto.bloodType.trimmed().toUpper();
  updateDto.emergencyContactName = dto.emergencyContactName.simplified();
  updateDto.emergencyContactPhone = dto.emergencyContactPhone.trimmed();
}
...
bool PatientService::addOutPatient(OutPatientInputDTO &dto) {
  ...
  OutPatientInsertDTO insertDto;
  populateBaseInsertDTO(insertDto, dto, patientCode);
  insertDto.allergies = parseAllergiesString(0, dto.allergies);
  insertDto.insurance = parseInsuranceString(0, dto.insurance);
  insertDto.doctorId = dto.doctorId;
  insertDto.status = "REGISTERED";
...
bool PatientService::addInPatient(InPatientInputDTO &dto) {
  ...
  InPatientInsertDTO insertDto;
  populateBaseInsertDTO(insertDto, dto, patientCode);
  insertDto.allergies = parseAllergiesString(0, dto.allergies);
  insertDto.insurance = parseInsuranceString(0, dto.insurance);
  insertDto.roomId = dto.roomId;
  insertDto.doctorId = dto.doctorId;
  insertDto.admissionDate = dto.admissionDate.toString("yyyy-MM-dd");
  insertDto.dischargeDate = dto.dischargeDate.has_value()
                                ? dto.dischargeDate->toString("yyyy-MM-dd")
                                : "";
  insertDto.reason = dto.reason;
  insertDto.status = "ADMITTED";
...
bool PatientService::addEmergencyPatient(EmergencyPatientInputDTO &dto) {
  ...
  EmergencyPatientInsertDTO insertDto;
  populateBaseInsertDTO(insertDto, dto, patientCode);
  insertDto.allergies = parseAllergiesString(0, dto.allergies);
  insertDto.insurance = parseInsuranceString(0, dto.insurance);
  insertDto.roomId = dto.roomId;
  insertDto.doctorId = dto.doctorId;
  insertDto.injuryCause = dto.injuryCause;
  insertDto.injuryDescription = dto.injuryDescription;
  insertDto.admissionDate = dto.admissionDate.toString("yyyy-MM-dd");
  insertDto.dischargeDate = dto.dischargeDate.has_value()
                                ? dto.dischargeDate->toString("yyyy-MM-dd")
                                : "";
  insertDto.status = "EMERGENCY";
...
QString PatientService::validateBaseInput(const PatientInputDTO &dto,
  ...
  QString PatientService::validateEmergencyDoctorId(
  ...
  QString PatientService::validateEmergencyDischargeDate(
    ...
    QString PatientService::validateEmergencyInjuryDescription(
        const QString &desc) {
    ...
    QString PatientService::validateUpdateBaseInput(const PatientInputDTO &dto,
      ...
      if (!(err = Validation::validateFullName(dto.fullName)).isEmpty())
        return err;
      if (!dto.dateOfBirth.isValid() ||
          dto.dateOfBirth > QDate::currentDate() ||
          dto.dateOfBirth.year() < QDate::currentDate().year() - 150)
        return "Ngày sinh không hợp lệ (tuổi phải nhỏ hơn 150).";
    ...
    bool PatientService::updateOutPatient(
        int patientId, OutPatientInputDTO &dto, const QString &status) {
      normalizePatientInput(dto);
      QString err = validateUpdateBaseInput(dto, patientId);
      if (!err.isEmpty()) {
        QMessageBox::warning(nullptr, "Validation Error", err);
      ...
      OutPatientUpdateDTO updateDto;
      populateBaseUpdateDTO(updateDto, dto, patientId);
    ...
    bool PatientService::updateInPatient(int patientId, InPatientInputDTO &dto,
      ...
      InPatientUpdateDTO updateDto;
      populateBaseUpdateDTO(updateDto, dto, patientId);
      updateDto.roomId = dto.roomId;
      updateDto.doctorId = dto.doctorId;
      updateDto.admissionDate = dto.admissionDate.toString("yyyy-MM-dd");
      updateDto.dischargeDate = dto.dischargeDate.has_value()
                                    ? dto.dischargeDate->toString("yyyy-MM-dd")
                                    : "";
      updateDto.reason = dto.reason;
      updateDto.status = status;
    ...
    bool PatientService::updateEmergencyPatient(
      ...
      EmergencyPatientUpdateDTO updateDto;
      populateBaseUpdateDTO(updateDto, dto, patientId);
      updateDto.roomId = dto.roomId;
      updateDto.doctorId = dto.doctorId;
      updateDto.injuryCause = dto.injuryCause;
      updateDto.injuryDescription = dto.injuryDescription;
      updateDto.admissionDate = dto.admissionDate.toString("yyyy-MM-dd");
      updateDto.dischargeDate = dto.dischargeDate.has_value()
                                    ? dto.dischargeDate->toString("yyyy-MM-dd")
                                    : "";
    ...
    bool PatientService::checkDrugAllergyConflict(
        int patientId, const QString &drugName) const {
      QString allergies =
