...
QStringList AppointmentService::getAvailableTimeSlots(int doctorId, const QDate &date) const {
    ...
    QList<AppointmentRecordDTO> bookedAppts = getDoctorAppointments(doctorId, date);
    ...
    for (const auto& slot : baseSlots) {
        ...
        for (const auto& appt : bookedAppts) {
            ...
            if (!apptStart.isValid()) apptStart = QTime::fromString(appt.startTime, "HH:mm:ss");
            
            QTime apptEnd = QTime::fromString(appt.endTime, "HH:mm");
            if (!apptEnd.isValid()) apptEnd = QTime::fromString(appt.endTime, "HH:mm:ss");

            if (!apptStart.isValid() || !apptEnd.isValid()) continue;
        ...
        if (!isBooked) {
            QString slotStr = slotStart.toString("HH:mm") + " - " + slotEnd.toString("HH:mm");
