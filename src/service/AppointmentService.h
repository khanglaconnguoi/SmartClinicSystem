...
private:
  ...
  // Aggregate validator
  QString validateAppointmentInput(const AppointmentInputDTO &input,
                                   int excludeId = -1) const;

public:
  explicit AppointmentService(
      std::shared_ptr<AppointmentRepository> appointmentRepo,
  ...
  QString validateDoctorConflict(int doctorId, const QDate &date,
                                 const QTime &startTime, const QTime &endTime,
                                 int excludeId = -1) const;
  QString validatePatientConflict(int patientId, const QDate &date,
                                  const QTime &startTime, const QTime &endTime,
                                  int excludeId = -1) const;
