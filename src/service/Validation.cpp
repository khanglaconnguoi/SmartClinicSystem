...
QString validateFullName(const QString& fullName) {
    if (fullName.trimmed().isEmpty()) { return "Full name is required."; }
    if (fullName.trimmed().length() > FULL_NAME_MAX_LENGTH) {
        return QString("Full name must not exceed %1 characters.").arg(FULL_NAME_MAX_LENGTH);
    }
    return "";
}
...
QString validateEmail(const QString& email) {
    if (email.trimmed().isEmpty()) { return "Email is required."; }
...
QString validateAddress(const QString& address) {
    if (address.trimmed().isEmpty()) { return "Address is required."; }
    return "";
}

QString validateDateOfBirth(const QDate& dateOfBirth) {
    if (dateOfBirth.isNull()) { return "Date of birth is required."; }
