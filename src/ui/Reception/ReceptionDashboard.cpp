...
ReceptionDashboardWidget::ReceptionDashboardWidget(std::shared_ptr<IAuthenticatable> user, QWidget *parent)
...
void ReceptionDashboardWidget::loadDoctorsBySpecialty(const QString& specialty) {
    ...
    auto doctors = repo.getDoctorsBySpecialty(specialty);
    for (const auto& doc : doctors) {
        // user data = staff_code, display text = full_name
        m_comboDoctor->addItem(doc.second, doc.first);
    }
}
