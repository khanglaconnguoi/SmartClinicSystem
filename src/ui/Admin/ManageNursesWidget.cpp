...
void ManageNursesWidget::buildUI() {
  ...
  QLabel *lblPageTitle = new QLabel("QUẢN LÝ Y TÁ", this);
  ...
  m_tblNurses->horizontalHeader()->resizeSection(4, 320);
  ...
  m_tblNurses->setFocusPolicy(Qt::NoFocus);
  ...
  connect(btnAddNew, &QPushButton::clicked, this,
          &ManageNursesWidget::showAddNurseDialog);
  connect(m_txtSearchKey, &QLineEdit::textChanged, this,
          &ManageNursesWidget::handleFilterChanged);
  connect(m_cbLevelFilter, QOverload<int>::of(&QComboBox::currentIndexChanged),
  ...
  connect(m_btnResetFilters, &QPushButton::clicked, this,
          &ManageNursesWidget::handleResetFilters);
  connect(m_btnPrevPage, &QPushButton::clicked, this,
          &ManageNursesWidget::handlePrevPage);
  connect(m_btnNextPage, &QPushButton::clicked, this,
          &ManageNursesWidget::handleNextPage);
  ...
  for (int i = 0; i < nurses.size(); ++i) {
    ...
    connect(btnResetPwd, &QPushButton::clicked, this, [this, nurse]() {
      auto confirm = QMessageBox::question(
          ...
          QString("Bạn có chắc chắn muốn reset mật khẩu cho Y tá %1 (%2)?")
...
void ManageNursesWidget::showEditNurseDialog(
    std::shared_ptr<SystemUser> nurse) {
  auto profile = m_staffService->getOwnProfile(nurse->getAccountId());
  auto nurseProfile = dynamic_cast<NurseProfileDTO *>(profile.get());
  if (nurseProfile) {
    NurseRegistrationDialog dialog(m_staffService, this);
