...
NurseRegistrationDialog::NurseRegistrationDialog(
    ...
    const int dlgH = qMin(840, avail.height() - 40);
...
void NurseRegistrationDialog::setupUi() {
    ...
    m_lblPageTitle = new QLabel("THÔNG TIN Y TÁ", headerFrame);
    ...
    m_txtFullName->setPlaceholderText("VD: Nguyễn Thị C");
    ...
    m_txtEmail->setPlaceholderText("VD: nurse@example.com");
    ...
    QFormLayout *form2 = new QFormLayout(gbJobInfo);
    ...
    QGroupBox *gbProfInfo = new QGroupBox("Thông tin chuyên môn", formCard);
    gbProfInfo->setStyleSheet(groupBoxStyle);
    QFormLayout *form3 = new QFormLayout(gbProfInfo);
    form3->setContentsMargins(16, 18, 16, 16);
    form3->setSpacing(12);
    form3->setLabelAlignment(Qt::AlignRight | Qt::AlignVCenter);

    m_cbNurseLevel = new QComboBox(gbProfInfo);
    ...
    m_txtCertification = new QLineEdit(gbProfInfo);
    m_txtCertification->setStyleSheet(extraInputStyle);
    m_txtCertification->setPlaceholderText("VD: Chứng chỉ điều dưỡng quốc gia");
    form3->addRow("Chứng chỉ (*):", m_txtCertification);

    cardLayout->addWidget(gbJobInfo);
    cardLayout->addWidget(gbProfInfo);
...
void NurseRegistrationDialog::loadNurseData(NurseProfileDTO* nurse) {
    ...
    m_cbGender->setCurrentText(GenderText::toVi(nurse->gender));
...
void NurseRegistrationDialog::setReadOnlyMode(bool readOnly) {
    ...
    m_txtCertification->setReadOnly(readOnly);
    ...
    m_cbNurseLevel->setEnabled(!readOnly);
    if (m_avatarPicker) m_avatarPicker->setEnabled(!readOnly);

    if (m_lblPageTitle) {
        m_lblPageTitle->setText(readOnly ? "THÔNG TIN CHI TIẾT Y TÁ"
                                         : (m_editStaffId != -1 ? "CHỈNH SỬA THÔNG TIN Y TÁ" : "THÊM Y TÁ MỚI"));
    }
...
void NurseRegistrationDialog::handleSave() {
    ...
    if (m_editStaffId == -1) {
        ...
        if (errorMsg.isEmpty()) {
            ...
            QLabel *lblTitle = new QLabel("Tạo tài khoản Y tá thành công!", &successDialog);
        ...
        errorMsg = m_staffService->editNurseInformation(dto, m_editStaffId);
        if (errorMsg.isEmpty()) {
            QMessageBox msgBox(this);
            msgBox.setWindowTitle("Thành công");
            msgBox.setText("Cập nhật thông tin Y tá thành công!");
