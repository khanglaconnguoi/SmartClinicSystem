...
void ManageLeaveWidget::setupUi() {
    ...
    m_tablePendingLeaves->setStyleSheet(
        "QTableWidget { border: 1px solid #E2E8F0; border-radius: 4px; color: #0F172A; background-color: white; outline: none; }"
    ...
    if (m_comboLeaveStaff->lineEdit()) {
        m_comboLeaveStaff->lineEdit()->setPlaceholderText("Nhập Mã NV hoặc Họ tên để tìm kiếm...");
    }
    m_comboLeaveStaff->setStyleSheet(
        "QComboBox { background: transparent; border: 1px solid #CBD5E1; border-radius: 6px; padding: 8px 12px; font-size: 13px; color: #0F172A; }"
        "QComboBox QAbstractItemView { background-color: #FFFFFF; color: #0F172A; selection-background-color: #2563EB; selection-color: white; }"
        "QLineEdit { background: transparent; color: #0F172A; border: none; font-size: 13px; }"
    );
    ...
    m_lblLeaveBalance = new QLabel("Vui lòng nhập hoặc chọn nhân viên.", formFrame);
    m_lblLeaveBalance->setStyleSheet("color: #5F6368; font-style: italic; background: transparent;");
    ...
    auto createCustomCalendar = []() {
        ...
        cal->setStyleSheet(
            ...
            "QCalendarWidget QWidget#qt_calendar_navigationbar { background-color: #F8FAFC; border-bottom: 1px solid #E2E8F0; min-height: 38px; }"
            "QCalendarWidget QToolButton { color: #1E293B; font-weight: bold; background-color: transparent; border: none; padding: 4px 8px; margin: 2px; font-size: 13px; border-radius: 4px; }"
            ...
            "QCalendarWidget QAbstractItemView:enabled { font-size: 13px; color: #1E293B; background-color: #FFFFFF; selection-background-color: #2563EB; selection-color: #FFFFFF; outline: none; }"
    ...
    QString transparentInputStyle =
        "QDateEdit, QTextEdit { background: transparent; border: 1px solid #CBD5E1; border-radius: 6px; padding: 8px 12px; font-size: 13px; color: #0F172A; }"
    ...
    if (m_comboLeaveStaff->lineEdit()) {
        connect(m_comboLeaveStaff->lineEdit(), &QLineEdit::textChanged, this, [this](const QString&) {
        ...
        if (!currentText.isEmpty()) {
            for (int i = 0; i < m_comboLeaveStaff->count(); ++i) {
                if (m_comboLeaveStaff->itemData(i).toInt() > 0 && 
                    m_comboLeaveStaff->itemText(i).contains(currentText, Qt::CaseInsensitive)) {
...
void ManageLeaveWidget::onSubmitLeaveRequest() {
    ...
    if (staffId <= 0) {
        QMessageBox::warning(this, "Lỗi", "Vui lòng nhập hoặc chọn nhân viên hợp lệ.");
        return;
    }
