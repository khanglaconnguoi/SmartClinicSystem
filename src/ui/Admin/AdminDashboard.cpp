...
AdminDashboardWidget::AdminDashboardWidget(
    ...
    : BaseDashboardWidget(user, staffService, parent),
      ...
      m_analyticService(analyticService), m_manageDoctorsPage(nullptr),
      m_manageNursesPage(nullptr), m_manageReceptionPage(nullptr),
      m_managePharmacistsPage(nullptr), m_analyticsPage(nullptr) {
  initializeDashboard();
}

void AdminDashboardWidget::fillDashboardData() {
  if (m_currentUser && m_nameLabel) {
    m_nameLabel->setText(("ADMIN: " + m_currentUser->getFullName()).toUpper());
  }
  ...
  m_btnManageReception = new QPushButton("Quản lý Lễ tân", m_sidebarFrame);
  m_btnManagePharmacists = new QPushButton("Quản lý Dược sĩ", m_sidebarFrame);
  m_btnManageLeaves = new QPushButton("Quản lý Nghỉ phép", m_sidebarFrame);
  ...
  m_manageDoctorsPage =
      new ManageDoctorsWidget(m_staffService, m_appointmentService, this);
  ...
  m_manageLeavesPage =
      new ManageLeaveWidget(m_staffService, m_appointmentService, this);
  ...
  auto setActiveMenu = [=](QPushButton *activeBtn) {
    QPushButton *buttons[] = {m_btnAnalytics,         m_btnManageDoctors,
                              m_btnManageNurses,      m_btnManageReception,
                              m_btnManagePharmacists, m_btnManageLeaves};
