...
ManageDoctorsWidget::ManageDoctorsWidget(std::shared_ptr<StaffService> staffService, QWidget* parent)
...
void ManageDoctorsWidget::loadDoctorsList() {
    if (!m_tblDoctors || !m_staffService) return;
    ...
    for (int i = 0; i < doctors.size(); ++i) {
        ...
        // Currently missing in simplified SystemUser
        ...
        if (doc->isActive()) {
            itemStatus->setForeground(QBrush(QColor("#059669"))); // Xanh la
        } else {
            itemStatus->setForeground(QBrush(QColor("#DC2626"))); // Do
        }
        ...
        connect(btnEdit, &QPushButton::clicked, this, [this, doc]() {
            showEditDoctorDialog(doc);
        });
    }
}
...
void ManageDoctorsWidget::showEditDoctorDialog(std::shared_ptr<Doctor> doc) {
    ...
    if (doctorProfile) {
        ...
        QMessageBox::warning(this, "Lỗi", "Không thể lấy thông tin chi tiết bác sĩ.");
    }
}
