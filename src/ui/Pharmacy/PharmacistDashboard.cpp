...
void PharmacistDashboardWidget::fillDashboardData() {
  ...
  if (m_currentUser && m_nameLabel) {
    m_nameLabel->setText(
        (m_currentUser->getFullName() + " (DƯỢC SĨ)").toUpper());
  }
...
void PharmacistDashboardWidget::selectInvoiceRow(int row) {
  ...
  if (optInv.has_value()) {
    ...
    m_lblBillPatientName->setText(
        QString("<b>Hóa đơn số:</b> %1").arg(inv.invoiceCode));
    m_lblBillDetails->setText(QString("Ngày lập: %1 | Trạng thái: %2")
...
void PharmacistDashboardWidget::showCreateInvoiceDialog(
    const PrescriptionResultDTO &presc, int patientId) {
  QDialog dlg(this);
  ...
  lay->addWidget(
      new QLabel(QString("<b>Bệnh nhân:</b> %1").arg(presc.patientName)));
  lay->addWidget(new QLabel(QString("<b>Tuổi:</b> %1 | <b>Giới tính:</b> %2")
                                .arg(presc.patientAge)
                                .arg(presc.patientGender)));
  lay->addWidget(
      new QLabel(QString("<b>Bác sĩ kê đơn:</b> %1").arg(presc.doctorName)));
  lay->addWidget(
      new QLabel(QString("<b>Ngày kê đơn:</b> %1")
                     .arg(presc.prescribedAt.toString("dd/MM/yyyy HH:mm"))));
  ...
  lay->addWidget(new QLabel(
      QString("<b>Tiền thuốc:</b> %1 VND")
          .arg(QLocale(QLocale::Vietnamese).toString(medTotal, 'f', 0))));
  ...
  auto updateGrandTotal = [spinFee, medTotal, lblGrandTotal]() {
    ...
    lblGrandTotal->setText(
        QString("<h3>Tổng thanh toán: <span style='color:#E53E3E;'>%1 "
                "VND</span></h3>")
            .arg(QLocale(QLocale::Vietnamese).toString(grand, 'f', 0)));
  };
  connect(spinFee, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
          updateGrandTotal);
  updateGrandTotal();
  ...
  btnSave->setStyleSheet(
      "background-color: #10B981; color: white; border: none; border-radius: "
      "4px; padding: 6px 12px; font-weight: bold;");
  ...
  connect(btnSave, &QPushButton::clicked, &dlg,
          [this, &dlg, presc, patientId, spinFee]() {
            bool success = m_billingService->createInvoice(
                patientId, presc.recordId, PatientType::Outpatient,
                spinFee->value(), presc.items);
            if (success) {
