...
ClinicalExamWidget::ClinicalExamWidget(
    std::shared_ptr<MedicalRecordService> medicalRecordService, QWidget *parent)
  ...
  connect(m_btnCancel, &QPushButton::clicked, this,
          &ClinicalExamWidget::onCancelExamClicked);
  connect(m_btnFinish, &QPushButton::clicked, this,
          &ClinicalExamWidget::onFinishExamClicked);
  if (m_btnCallPatient) {
    connect(m_btnCallPatient, &QPushButton::clicked, this,
            [this]() { emit callPatientRequested(m_currentAppointmentId); });
  }

  if (m_btnSave) {
    connect(m_btnSave, &QPushButton::clicked, this,
            &ClinicalExamWidget::onSaveClicked);
  }

  auto openPrescriptionAction = [this]() {
    if (m_currentMedicalRecordId <= 0) {
      QMessageBox::warning(this, "Chưa lưu bệnh án",
                           "Vui lòng 'Viết Hồ Sơ Bệnh Án' và lưu thành công "
                           "trước khi kê đơn thuốc!");
      return;
    }
    CreatePrescriptionDialog dialog(m_pharmacyService, m_patientService,
                                    m_currentPatientId, this);
    dialog.setRecordId(QString::number(m_currentMedicalRecordId));
    int doctorId = 1;
    QString doctorName = "Bác sĩ";
    if (UserSession::getInstance().isLoggedIn() &&
        UserSession::getInstance().getCurrentAccount()) {
      doctorId = UserSession::getInstance().getCurrentAccount()->getAccountId();
      doctorName =
          UserSession::getInstance().getCurrentAccount()->getFullName();
    }
  ...
  if (m_btnPrescription)
    connect(m_btnPrescription, &QPushButton::clicked, this,
            openPrescriptionAction);

  if (m_btnServiceOrder)
    connect(m_btnServiceOrder, &QPushButton::clicked, this,
            &ClinicalExamWidget::openLabRequestDialog);

  auto openHistoryAction = [this]() {
    PatientRecordHistoryDialog dialog(m_pharmacyService, m_medicalRecordService,
                                      this);
  ...
  auto onBmiInputChanged = [this]() { updateBmi(); };
  ...
  connect(m_txtTemp, &QLineEdit::textChanged, this,
          &ClinicalExamWidget::validateTemperatureInput);
  connect(m_txtPulse, &QLineEdit::textChanged, this,
          &ClinicalExamWidget::validateHeartRateInput);
  connect(m_txtWeight, &QLineEdit::textChanged, this,
          &ClinicalExamWidget::validateWeightInput);
  connect(m_txtHeight, &QLineEdit::textChanged, this,
...
void ClinicalExamWidget::openLabRequestDialog() {
  if (m_currentMedicalRecordId <= 0) {
    QMessageBox::warning(this, "Chưa lưu bệnh án",
                         "Vui lòng 'Viết Hồ Sơ Bệnh Án' và lưu thành công "
                         "trước khi gửi yêu cầu xét nghiệm!");
    return;
  }
  ...
  QString patName =
      m_lblPatientNameVal ? m_lblPatientNameVal->text() : "Bệnh nhân";
  QString patCode = m_lblPatientCodeVal ? m_lblPatientCodeVal->text() : "";

  CreateLabRequestDialog dialog(m_serviceRequestService, m_appointmentService,
                                m_currentMedicalRecordId, doctorId, doctorName,
                                patName, patCode, this);
  ...
  for (const auto &req : requests) {
    if (req.recordId == m_currentMedicalRecordId) {
      ...
      QString title = QString("[%1] %2 — Trạng thái: %3")
                          .arg(req.serviceName, req.roomName,
                               serviceRequestStatusToVi(req.status));
      ...
      summaryText += QString("[%1] %2 - Trạng thái: %3\n")
                         .arg(req.roomName, req.serviceName,
                              serviceRequestStatusToVi(req.status));
...
void ClinicalExamWidget::setupUi() {
  ...
  leftScroll->setWidget(setupMainExamForm());
  ...
  rightScroll->setStyleSheet(
      "QScrollArea { background: transparent; border: none; }");
  rightScroll->setWidget(setupMedicalHistoryPanel());

  workspaceLayout->addWidget(leftScroll, 6);
  workspaceLayout->addWidget(rightScroll, 4);
...
QFrame *ClinicalExamWidget::setupPatientInfoCard() {
  ...
  infoCard->setStyleSheet("QFrame#InfoCard { background-color: #FFFFFF; "
  ...
  avatarIcon->setStyleSheet(
      "font-size: 16px; font-weight: bold; color: #2563EB; background: "
  ...
  m_lblPatientCodeVal->setStyleSheet(
      "color: #111827; font-size: 12px; font-weight: bold; background: "
      "transparent;");

  QLabel *lblNameTitle = new QLabel("Họ Tên:", infoCard);
  lblNameTitle->setStyleSheet("color: #6B7280; font-size: 12px; font-weight: "
                              "600; background: transparent;");
  m_lblPatientNameVal = new QLabel("HOÀNG AN BÌNH", infoCard);
  m_lblPatientNameVal->setStyleSheet(
  ...
  m_lblPatientDobVal->setStyleSheet(
  ...
  m_lblPatientGenderVal->setStyleSheet(
      "color: #111827; font-size: 12px; font-weight: 600; background: "
      "transparent;");

  QLabel *lblAgeTitle = new QLabel("Tuổi:", infoCard);
  lblAgeTitle->setStyleSheet(
      "color: #6B7280; font-size: 12px; background: transparent;");
  m_lblPatientAgeVal = new QLabel("36 tuổi", infoCard);
  m_lblPatientAgeVal->setStyleSheet(
      "color: #111827; font-size: 12px; font-weight: 600; background: "
      "transparent;");

  QLabel *lblBloodTitle = new QLabel("Nhóm máu:", infoCard);
  lblBloodTitle->setStyleSheet(
      "color: #6B7280; font-size: 12px; background: transparent;");

  m_cbBloodType = new QComboBox(infoCard);
  m_cbBloodType->setFixedHeight(26);
  m_cbBloodType->setStyleSheet(
      "QComboBox { font-size: 12px; font-weight: bold; color: #111827; "
      "background-color: #FFFFFF; border: 1px solid #CBD5E1; border-radius: "
      "4px; padding: 2px 6px; }"
      "QComboBox:disabled { color: #475569; background-color: #F1F5F9; border: "
      "1px solid transparent; }"
      "QComboBox::drop-down { border: 0px; } QComboBox::down-arrow { image: "
      "url(:/icons/down_arrow.png); width: 10px; height: 10px; }");
  ...
  QPushButton *actionBtns[] = {m_btnSave,         m_btnPrescription,
                               m_btnServiceOrder, m_btnFinish,
                               m_btnCancel,       m_btnHistory};
  QString colors[] = {"#2563EB", "#7C3AED", "#0891B2",
                      "#059669", "#DC2626", "#4B5563"};

  for (int i = 0; i < 6; ++i) {
    actionBtns[i]->setCursor(Qt::PointingHandCursor);
    actionBtns[i]->setFixedHeight(34);
    actionBtns[i]->setStyleSheet(
        QString(
            "QPushButton { background-color: %1; color: white; border: none; "
            "font-size: 11px; font-weight: bold; border-radius: 6px; padding: "
            "0 12px; }"
  ...
  for (auto *m : metas) {
    m->setStyleSheet("color: #4B5563; font-size: 11px; background: "
                     "transparent; font-weight: 500;");
    metaRow->addWidget(m);
  }
  metaRow->addStretch();
...
QWidget *ClinicalExamWidget::setupMainExamForm() {
  ...
  mainForm->setStyleSheet(
      "QFrame#MainForm { background-color: #FFFFFF; border: 1px solid #E5E7EB; "
      "border-radius: 8px; }"
      "QLabel { font-size: 12px; font-weight: bold; color: #374151; border: "
      "none; background: transparent; }"
      "QLineEdit { border: 1px solid #D1D5DB; border-radius: 6px; padding: 6px "
      "10px; font-size: 12px; color: #111827; }"
      ...
      "10px; font-size: 12px; color: #111827; background-color: white; }"
  ...
  vitalBox->setStyleSheet("QFrame#VitalBox { background-color: #F9FAFB; "
                          "border: 1px solid #E5E7EB; border-radius: 8px; }"
  ...
  m_lblBmiVal->setStyleSheet(
      "font-weight: bold; color: #1E3A8A; font-size: 12px;");
  ...
  for (const auto &pair : ActionText::getList())
    m_cbAction->addItem(pair.second, pair.first);
  ...
  m_txtAdvice->setPlaceholderText(
      "Ví dụ: Uống thuốc sau ăn, tái khám đúng hẹn...");
  ...
  lblResults->setStyleSheet(
      "font-size: 12px; font-weight: bold; color: #111827; margin-top: 6px;");
...
QFrame *ClinicalExamWidget::setupMedicalHistoryPanel() {
  ...
  rightPanel->setStyleSheet(
      "QFrame#RightPanel { background-color: #FFFFFF; border: 1px solid "
      "#E5E7EB; border-radius: 8px; }"
      "QLabel { font-size: 12px; font-weight: bold; color: #111827; border: "
      "none; background: transparent; }"
      "QTextEdit { border: 1px solid #D1D5DB; border-radius: 6px; padding: 6px "
      "10px; font-size: 12px; color: #374151; }"
  ...
  m_txtHistoryPersonal->setPlaceholderText(
      "Mô tả tiền sử bệnh lý của bệnh nhân và gia đình...");
  m_txtHistoryPersonal->setMinimumHeight(80);
  rightLayout->addWidget(m_txtHistoryPersonal);

  // --- ALLERGIES SECTION ---
  QHBoxLayout *allergyHeader = new QHBoxLayout();
  QLabel *lblAllergyTitle = new QLabel("Dị ứng:", rightPanel);
  m_btnAddActiveAllergy = new QPushButton("+ Dị ứng hoạt chất", rightPanel);
  m_btnAddOtherAllergy = new QPushButton("+ Dị ứng khác", rightPanel);
  ...
  QString btnOtherStyle = "QPushButton { background-color: #0891B2; color: "
                          "white; border: none; border-radius: 4px; font-size: "
                          "11px; font-weight: bold; padding: 5px 12px; }"
                          "QPushButton:hover { background-color: #0E7490; }";
  m_btnAddActiveAllergy->setStyleSheet(btnActiveStyle);
  m_btnAddOtherAllergy->setStyleSheet(btnOtherStyle);
  m_btnAddActiveAllergy->setCursor(Qt::PointingHandCursor);
  m_btnAddOtherAllergy->setCursor(Qt::PointingHandCursor);

  allergyHeader->addWidget(lblAllergyTitle);
  allergyHeader->addStretch();
  allergyHeader->addWidget(m_btnAddActiveAllergy);
  allergyHeader->addWidget(m_btnAddOtherAllergy);
  rightLayout->addLayout(allergyHeader);

  m_tblAllergies = new QTableWidget(rightPanel);
  m_tblAllergies->setColumnCount(5);
  m_tblAllergies->setHorizontalHeaderLabels(
      {"Tên Dị Ứng", "Phân Loại", "Mức Độ", "Ghi Chú", "Thao Tác"});
  m_tblAllergies->verticalHeader()->setVisible(false);
  m_tblAllergies->setSelectionBehavior(QAbstractItemView::SelectRows);
  m_tblAllergies->setEditTriggers(QAbstractItemView::NoEditTriggers);
  m_tblAllergies->setMinimumHeight(130);
  m_tblAllergies->setStyleSheet(
      "QTableWidget { border: 1px solid #D1D5DB; border-radius: 6px; "
      "background-color: #FFFFFF; font-size: 11px; gridline-color: #F3F4F6; }"
      "QHeaderView::section { background-color: #F9FAFB; padding: 5px 6px; "
      "font-weight: bold; border: none; border-bottom: 1px solid #E5E7EB; "
      "color: #374151; font-size: 11px; }");
  m_tblAllergies->horizontalHeader()->setSectionResizeMode(
  ...
  rightLayout->addWidget(m_tblAllergies);

  connect(m_btnAddActiveAllergy, &QPushButton::clicked, this,
          &ClinicalExamWidget::openAddActiveIngredientAllergyDialog);
  connect(m_btnAddOtherAllergy, &QPushButton::clicked, this,
          &ClinicalExamWidget::openAddOtherAllergyDialog);
  ...
  m_txtHistoryIllness->setMinimumHeight(80);
  ...
  m_txtExamGeneral->setPlaceholderText(
      "Tình trạng hô hấp, tuần hoàn, tiêu hóa...");
  m_txtExamGeneral->setMinimumHeight(80);
  rightLayout->addWidget(m_txtExamGeneral);

  rightLayout->addWidget(
      new QLabel("Tóm tắt kết quả cận lâm sàng:", rightPanel));
  m_txtClsSummary = new QTextEdit(rightPanel);
  m_txtClsSummary->setPlaceholderText(
      "Tóm tắt kết quả xét nghiệm máu, X-Quang, siêu âm...");
  m_txtClsSummary->setMinimumHeight(80);
...
void ClinicalExamWidget::clearExamForm() {
  ...
  if (m_txtReason)
    m_txtReason->clear();
  if (m_txtMainDisease)
    m_txtMainDisease->clear();
  if (m_txtSubDisease)
    m_txtSubDisease->clear();
  if (m_txtAdvice)
    m_txtAdvice->clear();
  if (m_txtHistoryIllness)
    m_txtHistoryIllness->clear();
  if (m_txtHistoryPersonal)
    m_txtHistoryPersonal->clear();
  if (m_txtExamGeneral)
    m_txtExamGeneral->clear();
  if (m_txtClsSummary)
  ...
  if (m_cbBloodType) {
    m_cbBloodType->setCurrentText(BloodTypeText::toVi("UNKNOWN"));
    m_cbBloodType->setEnabled(true);
  }
...
void ClinicalExamWidget::loadPatientInfo(int patientId, int appointmentId,
  ...
  if (m_lblPatientNameVal)
    m_lblPatientNameVal->setText(name.toUpper());
  if (m_lblPatientCodeVal)
    m_lblPatientCodeVal->setText(id);

  int age = 30 + (id.length() % 25);
  if (m_lblPatientAgeVal)
    m_lblPatientAgeVal->setText(QString("%1 tuổi").arg(age));
  if (m_lblPatientGenderVal)
    m_lblPatientGenderVal->setText(age % 2 == 0 ? "Nam" : "Nữ");
  ...
  if (m_patientService && patientId > 0) {
    ...
    if (patientOpt.has_value()) {
      ...
      if (m_cbBloodType) {
        QString bt = pat.bloodType.trimmed().toUpper();
        if (bt.isEmpty() || bt == "UNKNOWN") {
          m_cbBloodType->setCurrentText(BloodTypeText::toVi("UNKNOWN"));
          m_cbBloodType->setEnabled(true);
        } else {
          m_cbBloodType->setCurrentText(BloodTypeText::toVi(bt));
          m_cbBloodType->setEnabled(false);
        }
      }
    }

    m_currentAllergies.clear();
    auto allergiesResult = m_patientService->getAllergies(patientId);
    for (const auto &a : allergiesResult) {
      AllergyInputDTO dto;
      dto.ingredientId = a.ingredientId;
      dto.allergenName = a.allergenName;
      dto.severity = a.severity;
      dto.notes = a.notes;
      m_currentAllergies.append(dto);
    }
    refreshAllergyTable();
  }
  ...
  if (m_medicalRecordService && appointmentId > 0) {
    auto recordOpt =
        m_medicalRecordService->getLatestRecordByAppointmentId(appointmentId);
    if (recordOpt.has_value()) {
      const auto &rec = recordOpt.value();
      m_currentMedicalRecordId = rec.recordId;

      if (m_txtTemp)
        m_txtTemp->setText(QString::number(rec.vitals.temperature));
      if (m_txtBp)
        m_txtBp->setText(rec.vitals.bloodPressure);
      if (m_txtPulse)
        m_txtPulse->setText(QString::number(rec.vitals.heartRate));
      if (m_txtWeight)
        m_txtWeight->setText(QString::number(rec.vitals.weight));
      if (m_txtHeight)
        m_txtHeight->setText(QString::number(rec.vitals.height));

      if (m_txtReason)
        m_txtReason->setText(rec.chiefComplaint);

      QRegularExpression rxNotes(
          "Bệnh sử: (.*?)\\nTiền sử: (.*?)\\nKhám lâm sàng chung: (.*?)\\nCận "
          "lâm sàng tóm tắt: (.*)",
      ...
      if (matchNotes.hasMatch()) {
        if (m_txtHistoryIllness)
          m_txtHistoryIllness->setText(matchNotes.captured(1).trimmed());
        if (m_txtHistoryPersonal)
          m_txtHistoryPersonal->setText(matchNotes.captured(2).trimmed());
        if (m_txtExamGeneral)
          m_txtExamGeneral->setText(matchNotes.captured(3).trimmed());
        if (m_txtClsSummary)
          m_txtClsSummary->setText(matchNotes.captured(4).trimmed());
      }

      QRegularExpression rxTreatment(
          "Hướng xử lý: (.*?)\\. Xử trí: (.*?)\\. Lời dặn: (.*)",
          QRegularExpression::DotMatchesEverythingOption);
      ...
      if (matchTreatment.hasMatch()) {
        ...
        if (m_txtAdvice)
          m_txtAdvice->setText(matchTreatment.captured(3).trimmed());
      }
...
void ClinicalExamWidget::loadPatientInfo(const PatientDetailDTO &patient,
                                         int appointmentId, const QString &time,
                                         const QString &specialty) {
  loadPatientInfo(patient.patientId, appointmentId, patient.fullName,
                  patient.patientCode, time, specialty);
...
void ClinicalExamWidget::updateBmi() {
  ...
  if (weight > 0 && heightCm > 0) {
    ...
    QString assessment;
    if (bmi < 18.5)
      assessment = "Cân nặng thấp (gầy)";
    else if (bmi < 25.0)
      assessment = "Bình thường";
    else if (bmi < 30.0)
      assessment = "Tiền béo phì";
    else
      assessment = "Béo phì";

    m_lblBmiVal->setText(
        QString("%1 (%2)").arg(QString::number(bmi, 'f', 2), assessment));
    m_lblBmiVal->setStyleSheet(QString("font-weight: bold; color: %1;")
                                   .arg(bmi < 25.0 ? "#059669" : "#DC2626"));
  } else {
...
QFrame *ClinicalExamWidget::createSeparator() {
  ...
  line->setStyleSheet(
      "background-color: #E5E7EB; border: none; max-height: 1px;");
  return line;
}
...
QList<Diagnosis> ClinicalExamWidget::getDiagnosesFromUi() const {
  ...
  if (!diagnosisDesc.isEmpty() || !mainDisease.isEmpty()) {
    ...
    d.severity = m_cbSeverity
                     ? DiagnosisSeverityText::toEn(m_cbSeverity->currentText())
                     : DiagnosisSeverityText::MODERATE;
    list.append(d);
  }
  return list;
}
...
void ClinicalExamWidget::validateTemperatureInput() {
  ...
  if (!err.isEmpty()) {
    m_txtTemp->setStyleSheet("QLineEdit { border: 1px solid #DC2626; "
...
void ClinicalExamWidget::validateHeartRateInput() {
  ...
  if (!err.isEmpty()) {
    m_txtPulse->setStyleSheet("QLineEdit { border: 1px solid #DC2626; "
                              "background-color: #FEF2F2; color: #111827; }");
...
void ClinicalExamWidget::validateWeightInput() {
  ...
  if (!err.isEmpty()) {
    m_txtWeight->setStyleSheet("QLineEdit { border: 1px solid #DC2626; "
                               "background-color: #FEF2F2; color: #111827; }");
...
void ClinicalExamWidget::validateHeightInput() {
  ...
  if (!err.isEmpty()) {
    m_txtHeight->setStyleSheet("QLineEdit { border: 1px solid #DC2626; "
                               "background-color: #FEF2F2; color: #111827; }");
...
void ClinicalExamWidget::validateChiefComplaintInput() {
  ...
  QString err = Validation::validateTrimmedNotEmpty(
      text, "Lý do khám không được để trống.");
  static const QString normalTextEditStyle =
      "QTextEdit { border: 1px solid #D1D5DB; border-radius: 6px; padding: 6px "
      "10px; font-size: 12px; color: #111827; background-color: #FFFFFF; }";
  if (!err.isEmpty()) {
    m_txtReason->setStyleSheet("QTextEdit { border: 1px solid #DC2626; "
                               "background-color: #FEF2F2; color: #111827; }");
...
void ClinicalExamWidget::onSaveClicked() {
  if (!m_medicalRecordService) {
    QMessageBox::critical(
        this, "Lỗi",
        "Chưa kết nối dịch vụ hồ sơ bệnh án (MedicalRecordService).");
    return;
  }

  // Save/update Blood Type if it was enabled and edited by doctor
  if (m_cbBloodType && m_cbBloodType->isEnabled() && m_currentPatientId > 0 &&
      m_patientService) {
    QString selectedBt = m_cbBloodType->currentText();
    QString bloodTypeEn = BloodTypeText::toEn(selectedBt);
    if (!bloodTypeEn.isEmpty() && bloodTypeEn != "UNKNOWN") {
      QString errBt =
          m_patientService->updateBloodType(m_currentPatientId, bloodTypeEn);
      if (!errBt.isEmpty()) {
        QMessageBox::warning(this, "Lỗi cập nhật nhóm máu", errBt);
        return;
      }
      m_cbBloodType->setEnabled(false);
    }
  ...
  // Save/update Allergies list for patient
  if (m_currentPatientId > 0 && m_patientService) {
    QString errAllergy = m_patientService->updateAllergiesOfPatient(
        m_currentPatientId, m_currentAllergies);
    if (!errAllergy.isEmpty()) {
      QMessageBox::warning(this, "Lỗi cập nhật danh sách dị ứng", errAllergy);
      return;
    }
  }
  ...
  dto.clinicalNotes = QString("Bệnh sử: %1\n"
                              "Tiền sử: %2\n"
                              "Khám lâm sàng chung: %3\n"
                              "Cận lâm sàng tóm tắt: %4")
                          .arg(m_txtHistoryIllness->toPlainText().trimmed(),
  ...
  if (m_currentMedicalRecordId > 0) {
    ...
    err = m_medicalRecordService->createMedicalRecord(
        dto, &m_currentMedicalRecordId);
  }
  if (!err.isEmpty()) {
  ...
  static const QString lineEditStyle =
      "QLineEdit { border: 1px solid #D1D5DB; border-radius: 6px; padding: 6px "
      "10px; font-size: 12px; color: #111827; background-color: #FFFFFF; }";
  static const QString comboStyle =
      "QComboBox { border: 1px solid #D1D5DB; border-radius: 6px; padding: 6px "
      "10px; font-size: 12px; color: #111827; background-color: #FFFFFF; }";
  static const QString textEditStyle =
      "QTextEdit { border: 1px solid #D1D5DB; border-radius: 6px; padding: 6px "
      "10px; font-size: 12px; color: #111827; background-color: #FFFFFF; }";
...
void ClinicalExamWidget::onFinishExamClicked() {
  if (m_currentMedicalRecordId <= 0) {
    QMessageBox::warning(
        this, "Chưa lưu bệnh án",
        "Hồ sơ bệnh án chưa được khởi tạo hoặc lưu. Vui lòng nhấn 'Viết Hồ Sơ "
        "Bệnh Án' để lưu trước khi hoàn tất khám!");
    return;
  }
...
void ClinicalExamWidget::onCancelExamClicked() {
  ...
  reply = QMessageBox::question(
      this, "Xác nhận hủy",
      "Bạn có chắc chắn muốn hủy cuộc hẹn này không?\n\nTrạng thái cuộc hẹn sẽ "
      "được cập nhật thành 'Đã hủy' (CANCELLED).",
      QMessageBox::Yes | QMessageBox::No);

  if (reply == QMessageBox::Yes) {
    if (m_appointmentService) {
      m_appointmentService->updateAppointmentStatus(
          m_currentAppointmentId, AppointmentStatusText::CANCELLED);
      QMessageBox::information(this, "Thành công",
                               "Đã hủy cuộc hẹn thành công.");
    } else {
      QMessageBox::warning(this, "Lỗi",
                           "Không thể kết nối với dịch vụ cuộc hẹn.");
    }
    emit backToDashboardRequested();
  }
}

void ClinicalExamWidget::refreshAllergyTable() {
  if (!m_tblAllergies)
    return;
  m_tblAllergies->setRowCount(0);

  int row = 0;
  for (int i = 0; i < m_currentAllergies.size(); ++i) {
    const auto &a = m_currentAllergies.at(i);
    m_tblAllergies->insertRow(row);

    QTableWidgetItem *nameItem = new QTableWidgetItem(a.allergenName);
    nameItem->setForeground(QBrush(QColor("#0F172A")));
    nameItem->setFont(QFont("Segoe UI", 9, QFont::Bold));

    QString typeStr = a.ingredientId.has_value() ? "Hoạt chất" : "Dị ứng khác";
    QTableWidgetItem *typeItem = new QTableWidgetItem(typeStr);
    typeItem->setTextAlignment(Qt::AlignCenter);
    typeItem->setForeground(
        QBrush(QColor(a.ingredientId.has_value() ? "#2563EB" : "#0891B2")));

    QString sevStr = severityToVi(a.severity);
    QTableWidgetItem *sevItem = new QTableWidgetItem(sevStr);
    sevItem->setTextAlignment(Qt::AlignCenter);
    sevItem->setForeground(QBrush(QColor("#1E293B")));

    QTableWidgetItem *notesItem =
        new QTableWidgetItem(a.notes.isEmpty() ? "--" : a.notes);
    notesItem->setForeground(QBrush(QColor("#475569")));

    QPushButton *btnDelete = new QPushButton("Xóa", this);
    btnDelete->setCursor(Qt::PointingHandCursor);
    btnDelete->setStyleSheet(
        "QPushButton { background-color: #EF4444; color: white; border: none; "
        "border-radius: 4px; padding: 2px 6px; font-size: 11px; font-weight: "
        "bold; }"
        "QPushButton:hover { background-color: #DC2626; }");

    QString nameToDel = a.allergenName;
    connect(btnDelete, &QPushButton::clicked, this, [this, nameToDel]() {
      for (int k = 0; k < m_currentAllergies.size(); ++k) {
        if (m_currentAllergies[k].allergenName.compare(
                nameToDel, Qt::CaseInsensitive) == 0) {
          m_currentAllergies.removeAt(k);
          break;
        }
      }
      refreshAllergyTable();
    ...
    m_tblAllergies->setItem(row, 0, nameItem);
    m_tblAllergies->setItem(row, 1, typeItem);
    m_tblAllergies->setItem(row, 2, sevItem);
    m_tblAllergies->setItem(row, 3, notesItem);
    m_tblAllergies->setCellWidget(row, 4, btnDelete);

    row++;
  }
}

void ClinicalExamWidget::openAddActiveIngredientAllergyDialog() {
  if (!m_pharmacyService) {
    QMessageBox::warning(this, "Lỗi",
                         "Chưa kết nối dịch vụ nhà thuốc (PharmacyService).");
    return;
  }

  AddActiveIngredientAllergyDialog dialog(m_pharmacyService, this);
  if (dialog.exec() == QDialog::Accepted) {
    AllergyInputDTO dto = dialog.getAllergyDTO();
    for (const auto &existing : m_currentAllergies) {
      if (existing.allergenName.compare(dto.allergenName,
                                        Qt::CaseInsensitive) == 0) {
        QMessageBox::warning(
            this, "Trùng lặp",
            QString("Bệnh nhân đã có dị ứng với hoạt chất '%1'.")
                .arg(dto.allergenName));
        return;
      }
    }
    m_currentAllergies.append(dto);
...
void ClinicalExamWidget::openAddOtherAllergyDialog() {
  AddOtherAllergyDialog dialog(this);
  if (dialog.exec() == QDialog::Accepted) {
    AllergyInputDTO dto = dialog.getAllergyDTO();
    for (const auto &existing : m_currentAllergies) {
      if (existing.allergenName.compare(dto.allergenName,
        ...
        QMessageBox::warning(
            this, "Trùng lặp",
            QString("Bệnh nhân đã có dị ứng với '%1'.").arg(dto.allergenName));
        return;
      }
    }
    m_currentAllergies.append(dto);
